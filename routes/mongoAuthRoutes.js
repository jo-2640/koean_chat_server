...
router.get('/getUserData', isAuthenticated, async (req, res) => {
  ...
  console.log('➡️ 요청 받은 시각:', new Date().toISOString());

  try {
    const currentUserId = req.user.uid;
    console.log(`🔑 인증 성공, currentUserId: ${currentUserId}`);

    // 1. 현재 사용자 정보 (친구 ID는 팝ulate하지 않음)
    console.log('🔍 1. 현재 사용자 정보 조회 시작...');
    const currentUser = await User.findById(currentUserId);
    console.log(`✅ 1. 사용자 조회 완료: ${currentUser ? currentUser.nickname : '사용자 없음'}`);

    if (!currentUser) {
      console.log('🚨 1. 사용자 없음 -> 404 응답');
      return res.status(404).json({ message: '사용자를 찾을 수 없습니다.' });
    }

    // 2. 친구 목록 (friendship 컬렉션에서 가져옴)
    console.log('🔍 2. 친구 목록 조회 시작...');
    const friendships = await Friendship.find({
      $or: [{ requester: currentUserId, status: 'accepted' }, { recipient: currentUserId, status: 'accepted' }],
    }).populate('requester', '_id nickname profileImgUrl gender')
      .populate('recipient', '_id nickname profileImgUrl gender');
    console.log(`✅ 2. 친구 목록 조회 완료, ${friendships.length}개 발견`);

    const friends = friendships.map(fs => {
      const friend = fs.requester._id.toString() === currentUserId ? fs.recipient : fs.requester;
      return friend;
    });
    console.log(`✅ 2. 친구 목록 매핑 완료, ${friends.length}명`);

    // 3. 알림 목록
    console.log('🔍 3. 알림 목록 조회 시작...');
    const notifications = await Notification.find({
      recipientId: currentUserId,
      isRead: false
    }).populate('senderId', '_id gender nickname profileImgUrl').sort({ createdAt: -1 });
    console.log(`✅ 3. 알림 목록 조회 완료, ${notifications.length}개 발견`);
